...
import bannerRules from "../assets/banner-rules.png";
import bannerCongrats from "../assets/banner-congrats.png";
import bannerTryAgain from "../assets/banner-tryagain.png";

function Banner({ image, eyebrow }) {
  return (
    <div className="relative h-48 overflow-hidden rounded-t-2xl">
      <img src={image} alt={eyebrow} className="h-full w-full object-cover" />
      {eyebrow && (
        <span className="absolute left-4 top-4 rounded-full bg-black/40 px-3 py-1 text-xs font-semibold tracking-[.15em] text-white backdrop-blur">
          {eyebrow}
        </span>
      )}
    </div>
...
export default function Learning() {
  ...
  const courseTitle = order?.courseTitle || "Kelas Videobelajar";
  const courseCategory = order?.courseCategory || "materi ini";
  const videoModuleLabel = `Foundation ${courseTitle}`;
  ...
  // Video pembelajarannya belum benar-benar ada (mock project), jadi
  // tombol play diarahkan ke halaman "404 Video Tidak Ditemukan" yang
  // menawarkan tutorial pemula sesuai kategori/kelas ini di YouTube.
  const goToVideoNotFound = () => {
    navigate("/video-not-found", {
      state: { courseTitle, category: courseCategory, backTo: `/learning/${orderId}` },
    });
  };

  const Shell = ({ children }) => <LearningShell courseTitle={courseTitle} screen={screen}>{children}</LearningShell>;

  if (screen === "video") return <Shell><div className="overflow-hidden rounded-2xl border bg-white shadow-sm"><div className="grid lg:grid-cols-[1fr_280px]"><section><div className="flex aspect-video items-center justify-center bg-slate-900"><button onClick={goToVideoNotFound} className="grid h-20 w-20 place-items-center rounded-full bg-white text-3xl text-emerald-600 shadow-lg">▶</button></div><div className="p-7"><p className="text-sm font-semibold text-emerald-600">4.1 VIDEO PEMBELAJARAN</p><h1 className="mt-1 text-2xl font-bold text-slate-900">Pengantar {courseTitle}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">Pelajari dasar-dasar {courseTitle}, pahami konsep inti di bidang {courseCategory}, dan siapkan dirimu sebelum mengikuti pre-test.</p><button onClick={() => setScreen("rules")} className="mt-6 rounded-lg bg-emerald-500 px-5 py-3 text-sm font-semibold text-white hover:bg-emerald-600">Lanjut ke materi berikutnya →</button></div></section><aside className="border-l bg-slate-50 p-5"><h2 className="font-bold text-slate-800">Daftar Modul</h2><div className="mt-4 space-y-3 text-sm"><button className="w-full rounded-lg border border-emerald-300 bg-emerald-50 p-3 text-left font-medium text-emerald-700">✓ 4.1 Video: {videoModuleLabel}</button><button onClick={() => setScreen("rules")} className="w-full rounded-lg border bg-white p-3 text-left hover:border-emerald-400">4.2 {videoModuleLabel}</button><button className="w-full rounded-lg border bg-white p-3 text-left text-slate-500">Pre-Test: {QUESTIONS.length} Pertanyaan</button></div></aside></div><button onClick={() => setScreen("rules")} className="w-full bg-emerald-500 px-5 py-4 text-left text-sm font-semibold text-white hover:bg-emerald-600">{videoModuleLabel} <span className="float-right">→</span></button></div></Shell>;

  if (screen === "rules") return <Shell><div className="overflow-hidden rounded-2xl border bg-white shadow-sm"><Banner image={bannerRules} eyebrow="4.2 • PRE-TEST" /><div className="max-w-2xl p-8"><h2 className="text-xl font-bold">Aturan Pre-Test</h2><p className="mt-3 text-sm leading-6 text-slate-600">Pre-test ini mengukur pemahaman awal Anda mengenai dasar {courseTitle}.</p><ul className="mt-5 list-disc space-y-2 pl-5 text-sm text-slate-600"><li>Terdiri dari {QUESTIONS.length} pertanyaan pilihan ganda.</li><li>Pilih satu jawaban terbaik pada setiap pertanyaan.</li><li>Nilai kelulusan minimal adalah 80.</li><li>Anda dapat mengulang jika nilai belum mencapai batas kelulusan.</li></ul><button onClick={() => setScreen("quiz")} className="mt-7 rounded-lg bg-emerald-500 px-5 py-3 text-sm font-semibold text-white hover:bg-emerald-600">Mulai Pre-Test</button></div></div></Shell>;

  if (screen === "passed" || screen === "retry") {
    const passed = screen === "passed";
    return <Shell><div className="overflow-hidden rounded-2xl border bg-white shadow-sm"><Banner image={passed ? bannerCongrats : bannerTryAgain} eyebrow={passed ? "PRE-TEST SELESAI" : "NILAI BELUM MEMENUHI BATAS"} /><div className="p-8 text-center"><div className={`mx-auto grid h-20 w-20 place-items-center rounded-full text-3xl ${passed ? "bg-emerald-100 text-emerald-600" : "bg-orange-100 text-orange-600"}`}>{passed ? "✓" : "↻"}</div><h2 className="mt-5 text-2xl font-bold">{passed ? "Selamat, Anda lulus pre-test!" : "Sedikit lagi!"}</h2><p className="mt-2 text-sm text-slate-600">Nilai Anda: <b>{score}</b> / 100 · Batas kelulusan: 80</p><p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-slate-600">{passed ? `Anda sudah siap melanjutkan pembelajaran ${courseTitle}. Sertifikat kelulusan pre-test sudah bisa diunduh.` : "Nilai Anda belum mencapai 80. Pelajari materi kembali, lalu ulangi pre-test untuk mencoba lagi."}</p><div className="mt-7 flex flex-wrap justify-center gap-3">{passed ? <><button onClick={() => navigate(`/certificate/${orderId}`)} className="rounded-lg bg-amber-500 px-5 py-3 text-sm font-semibold text-white hover:bg-amber-600">🏆 Lihat Sertifikat</button><button onClick={() => navigate("/my-courses")} className="rounded-lg bg-emerald-500 px-5 py-3 text-sm font-semibold text-white">Kembali ke Kelas Saya</button></> : <button onClick={retry} className="rounded-lg bg-emerald-500 px-5 py-3 text-sm font-semibold text-white">Ulangi Pre-Test</button>}<button onClick={() => setScreen("rules")} className="rounded-lg border border-emerald-500 px-5 py-3 text-sm font-semibold text-emerald-600">Lihat Aturan</button></div></div></div></Shell>;
  }
